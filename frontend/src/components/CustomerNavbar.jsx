...
const CustomerNavbar = React.memo(function CustomerNavbar({ onLogout }) {
  const user = useMemo(() => {
    try { return JSON.parse(localStorage.getItem('user') || '{}'); } catch { return {}; }
  }, []);
  const email = user?.email || 'customer@ekrishihub';
      ...
      <nav className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <Link to="/customer-dashboard" className="flex items-center gap-2">
          ...
          <span className="text-lg sm:text-xl font-bold text-green-700 tracking-tight">
            eKrishiHub <span className="text-gray-400">Customer</span>
          </span>
        </Link>

        {/* Tabs */}
        <ul className="hidden md:flex items-center gap-6 text-sm">
          <li><NavLink to="/customer-dashboard" className={({isActive}) => isActive ? active : idle}>Dashboard</NavLink></li>
          <li><NavLink to="/products"            className={({isActive}) => isActive ? active : idle}>Products</NavLink></li>
          <li><NavLink to="/my-orders"           className={({isActive}) => isActive ? active : idle}>My Orders</NavLink></li>
        </ul>
